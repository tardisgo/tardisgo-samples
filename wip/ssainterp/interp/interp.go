...
//
func Interpret(mainpkg *ssa.Package, mode Mode, sizes types.Sizes, filename string, args []string, ext *Externals) (context *Context, exitCode int) {
	if ext == nil {
		ext = NewExternals()
	}
	i := &interpreter{
...
func (cont *Context) Call(name string, args []Ivalue) Ivalue {
	// TODO find a faster way than itterating over the packages/members
	fr := &frame{i: cont.i}
	pkgs := cont.i.prog.AllPackages()
	for _, pkg := range pkgs {
		for _, mem := range pkg.Members {
			targetFn, isFn := mem.(*ssa.Function)
			if isFn {
				if targetFn.String() == name {
					return call(cont.i, fr, token.NoPos, targetFn, args)
				}
			}
		}
	}
