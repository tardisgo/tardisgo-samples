...
//	Jn(n, ±Inf) = 0
//	Jn(n, NaN) = NaN
func myJn(n int, x float64) float64 {
	const (
...
//	Y1(n, x < 0) = NaN
//	Y1(n, NaN) = NaN
func myYn(n int, x float64) float64 {
	fmt.Println("myYn", n, x)
	...
	if x >= Two302 { // x > 2**302
		...
		b = Y1(x)
		fmt.Println(a, b)
		fmt.Println(a-0.2703669782660475, b-0.024250317979323227)
		// quit if b is -inf
		for i := 1; i < n && !IsInf(b, -1); i++ {
			fmt.Println("in:", i, a, b)
			i2 := i + i
			fi2 := float64(i2)
			fi2x := fi2 / x
			fi2xb := fi2x * b
			fi2xba := fi2xb - a
			fmt.Println(i2, fi2, fi2x, fi2xb, fi2xba)
			fmt.Println(fi2xba == ((float64(i+i)/x)*b - a))
			a = b
			b = fi2xba
			//a, b = b, (float64(i+i)/x)*b-a

			fmt.Println("out:", i, a, b)
		}
	}
	if sign {
		fmt.Println("=", -b)
